...
in {
  ...
  config = let
    ...
    mkSkillEntry = name: content:
      if isPathLikeContent content && lib.pathIsDirectory content
      then
    ...
    mkInstalledMarketplaceEntry = name: content:
  ...
  in
    lib.mkIf cfg.enable {
      ...
      programs.claude-code.finalPackage = let
      ...
      home = {
        ...
        file = lib.mkMerge [
          ...
          (lib.mkIf (cfg.marketplaces != {}) {
            "${cfg.configDir}/plugins/known_marketplaces.json".source = jsonFormat.generate "claude-code-known-marketplaces.json" (
