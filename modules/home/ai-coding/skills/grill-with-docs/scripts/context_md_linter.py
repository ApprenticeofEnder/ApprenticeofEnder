...
def lint(text: str) -> Dict[str, Any]:
    ...
    if h1_line_index is None:
        add(
            "h1-present",
            "FAIL",
            "No H1 (top-level '# Title') found. CONTEXT.md must start with the context name as H1.",
        )
    else:
    ...
    if h1_line_index is not None:
        ...
        if not desc:
            add(
                "description-present",
                "FAIL",
                "No description sentence between the H1 and the first ## section.",
            )
            ...
            add(
                "description-present",
                "PASS",
                f"Description present ({sentence_count} sentence(s)).",
            )
    ...
    if "language" not in sections:
        add(
            "language-section",
            "FAIL",
            "No '## Language' section found. This is the required core of CONTEXT.md.",
        )
    ...
    if not terms:
        ...
        add(
            "language-terms",
            "PASS",
            f"Detected {len(terms)} term(s) in Language section.",
        )
        for term, definition, avoid in terms:
            # Rule 5: definition exists
            if (
                not definition
                or definition.startswith("_Avoid_")
    ...
    if "flagged ambiguities" in sections:
        add(
            "flagged-ambiguities",
            "PASS",
            "'## Flagged ambiguities' section found (optional but useful).",
        )
...
def main(argv: List[str]) -> int:
    ...
    parser.add_argument(
        "--sample", action="store_true", help="Lint the embedded sample CONTEXT.md"
    )
