...
def scan_directory(adr_dir: Path) -> Dict[str, Any]:
    ...
    def add(file: str, rule: str, level: str, message: str) -> None:
        findings.append(
            {"file": file, "rule": rule, "level": level, "message": message}
        )
    ...
    if not md_files:
        add(
            ...
            "directory",
            "WARN",
            "Directory is empty — no ADRs scanned. Create lazily when the first ADR is needed.",
        )
        return finalize(findings, 0)
    ...
    for p in md_files:
        ...
        if not m:
            add(
                ...
                "filename-pattern",
                "FAIL",
                "Filename does not match NNNN-slug.md pattern. Expected e.g. 0001-event-sourced-orders.md.",
            )
            continue
        ...
        add(
            ...
            "filename-pattern",
            "PASS",
            f"Filename matches pattern (number={number:04d}).",
        )
    ...
    if files:
        ...
        if gaps:
            add(
                ...
                "numbering-gap",
                "WARN",
                f"Number gap(s) in sequence: {', '.join(f'{g:04d}' for g in gaps)}. Either commit withdrawn ADRs as 'proposed → withdrawn' or renumber.",
            )
        else:
            add(
                "(root)",
                "numbering-sequence",
                "PASS",
                f"Sequential numbering 0001..{files[-1][0]:04d} with no gaps.",
            )
    ...
    for number, name, path in files:
        ...
        if not after_h1:
            add(
                ...
                "body-non-empty",
                "FAIL",
                "ADR has H1 but no body. The 1-3 sentence context+decision is required.",
            )
        else:
            ...
            if word_count < 10:
                add(
                    ...
                    "body-non-empty",
                    "WARN",
                    f"ADR body is very short ({word_count} words). Confirm context+decision+why are all stated.",
                )
            else:
                add(
                    name,
                    "body-non-empty",
                    "PASS",
                    f"Body present ({word_count} words).",
                )
        ...
        if status:
            if status in VALID_STATUSES:
                ...
                if target in numbers_present:
                    add(
                        name,
                        "status-supersede-target",
                        "PASS",
                        f"Superseded by ADR-{target:04d} which exists.",
                    )
                else:
                    add(
                        name,
                        "status-supersede-target",
                        "FAIL",
                        f"Superseded by ADR-{target:04d} but that ADR is not present in this directory.",
                    )
            else:
                add(
                    name,
                    "status-frontmatter",
                    "FAIL",
                    f"Status '{status}' is not one of {sorted(VALID_STATUSES)} or 'superseded by ADR-NNNN'.",
                )
...
def main(argv: List[str]) -> int:
    ...
    parser.add_argument(
        "--sample", action="store_true", help="Scan the embedded sample ADR layout"
    )
