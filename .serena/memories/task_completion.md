...
- The `.devenv.flake.nix` is auto-generated by devenv -- DO NOT modify it directly.
...
- When adding a new program, choose between:
  - Single file (`program.nix`) for simple configs
  - Directory (`program/default.nix` + supporting files) for complex configs with themes/scripts
- When adding a new toolkit, remember to explicitly import it in the relevant `configurations/home/*.nix` files.
- When adding a new service, it will be auto-imported unless it has a `.bak.nix` suffix.
